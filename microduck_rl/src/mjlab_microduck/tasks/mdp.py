...
# =============================================================================
# Book courier — pick → carry → place a duck-scale paperback
# =============================================================================
#
# Phase (GroundPickPhaseCommand, period 8 s):
#   [0, PICK_END)     crouch, mouth to book
#   [PICK_END, CARRY_END)  walk the grasped book toward the seated reader
#   [CARRY_END, 1)    set the book down at the reader's feet
#
...
COURIER_GRASP_DIST = 0.045
COURIER_PLACE_DIST = 0.10
COURIER_BOOK_HALF_Z = 0.006


def _courier_phase(env: ManagerBasedRlEnv) -> torch.Tensor:
...
def _courier_buffers(env: ManagerBasedRlEnv) -> None:
    n = env.num_envs
    dev = env.device
...
def reset_courier_props(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor,
    book_offset: tuple = (0.16, 0.0),
    person_offset: tuple = (0.55, 0.0),
    book_noise_xy: float = 0.02,
...
def courier_update_grasp(
    ...
    sid = asset_cfg.site_ids[0]
    mouth = robot.data.site_pos_w[:, sid, :]
    book_pos = book.data.root_link_pos_w
    dist_mouth = torch.linalg.norm(mouth - book_pos, dim=-1)
    dist_person = torch.linalg.norm(book_pos[:, :2] - env._courier_person_xy, dim=-1)

    in_pick = phase < COURIER_PICK_END
    ...
    if held.any():
        ids = torch.where(held)[0]
        pose = torch.zeros(len(ids), 7, device=env.device)
        pose[:, :3] = mouth[ids]
        pose[:, 3] = 1.0
        book.write_root_link_pose_to_sim(pose, ids)
        book.write_root_link_velocity_to_sim(
            torch.zeros(len(ids), 6, device=env.device), ids
        )

    return torch.zeros(env.num_envs, device=env.device)


def courier_pick_proximity(
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", site_names=["mouth_tip"]),
    book_name: str = "book",
    std: float = 0.06,
...
def courier_place_success(
    ...
    book: Entity = env.scene[book_name]
    phase = _courier_phase(env)
    xy = torch.linalg.norm(book.data.root_link_pos_w[:, :2] - env._courier_person_xy, dim=-1)
...
def courier_book_pos_in_base(
    ...
    """Book position in the robot base frame. Critic-only."""
    robot: Entity = env.scene["robot"]
    book: Entity = env.scene[asset_name]
    rel = book.data.root_link_pos_w - robot.data.root_link_pos_w
    rot = matrix_from_quat(robot.data.root_link_quat_w)
    return torch.bmm(rot.transpose(1, 2), rel.unsqueeze(-1)).squeeze(-1)


def courier_person_pos_in_base(
    env: ManagerBasedRlEnv,
) -> torch.Tensor:
    """Reader XY (z=0) in the robot base frame. Critic-only."""
    _courier_buffers(env)
    robot: Entity = env.scene["robot"]
    person = torch.zeros(env.num_envs, 3, device=env.device)
    person[:, :2] = env._courier_person_xy
    person[:, 2] = env.scene.terrain.env_origins[:, 2]
    rel = person - robot.data.root_link_pos_w
    rot = matrix_from_quat(robot.data.root_link_quat_w)
    return torch.bmm(rot.transpose(1, 2), rel.unsqueeze(-1)).squeeze(-1)


def courier_grasp_flag(
    env: ManagerBasedRlEnv,
) -> torch.Tensor:
