...
MICRODUCK_APARTMENT_XML = "src/mjlab_microduck/robot/microduck/scene_apartment.xml"
...
def main():
    ...
    parser.add_argument("--apartment", action="store_true", help="Load the book-courier apartment scene (floor, paperback, seated reader)")
    ...
    if args.roller:
        ...
        xml_path = MICRODUCK_APARTMENT_XML
    ...
    if book_jid >= 0:
        badr = int(model.jnt_qposadr[book_jid])
        data.qpos[badr:badr + 7] = [0.16, 0.0, 0.006, 1.0, 0.0, 0.0, 0.0]
        mujoco.mj_forward(model, data)
        print("Apartment book placed at (0.16, 0, 0.006)")
